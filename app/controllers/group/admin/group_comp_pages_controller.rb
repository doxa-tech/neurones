...
class Group::Admin::GroupCompPagesController < ApplicationController
  before_filter :find_page, only: [:up, :down]
  before_filter :is_first?, only: [:up]
  before_filter :is_last?, only: [:down]
  
  def new
    @page = Group::Page.find(params[:group_page_id])
    @page.comp_groups << Group::CompGroup.find(params[:comp_group_id])
    @comp_page = Group::CompPage.last
  ...
  def destroy
    @comp_page = Group::CompPage.find(params[:id])
    @comp_page.destroy
    respond_to do |format|
      format.html
      format.js
