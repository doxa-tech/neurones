	...
	def connected?
		...
		if !current_user.nil?
			...
			when 'index'
				...
				@update_ownerships = Ownership.where('user_id = ? AND element_id = ? AND right_update = ?', current_user.id, element_id, true)
				@delete_ownerships = Ownership.where('user_id = ? AND element_id = ? AND right_delete = ?', current_user.id, element_id, true)
			when 'new' 
				@ownerships_all = Ownership.where('user_id = ? AND element_id = ? AND right_create = ?', current_user.id, element_id, true )
